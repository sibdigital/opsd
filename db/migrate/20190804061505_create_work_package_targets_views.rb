  ...
  def change
    execute <<-SQL
      ...
      create or replace view v_quartered_work_package_targets_with_quarter_groups as (
        select ROW_NUMBER () OVER (ORDER BY project_id, target_id, work_package_id, year) as id,
               project_id,
               target_id,
               work_package_id,
               year,
        ...
        group by project_id, target_id, work_package_id, year
      )
      ;
      ...
      create or replace view v_yeared_project_targets_fact_with_quarter_groups as (
             ...
             from v_quartered_work_package_targets_with_quarter_groups
             group by project_id,
                      target_id,
                      year
      );
