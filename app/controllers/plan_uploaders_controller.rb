...
class PlanUploadersController < ApplicationController
  ...
  def load_msproject
    ...
    settings = PlanUploaderSetting.select('column_name, column_num, is_pk')
                 .where("setting_type = 'UploadPlanType6'")
                 .order('column_num ASC').all
    ...
    xlsx.each_row_streaming(offset: @first_row_num.to_i - 1) do |row|
      ...
      #fixed columns
      rr['id'] = Hash['column_name', 'id', 'id', xlsx.excelx_value(row_num, 1), 'is_pk', 0]
    ...
    rows.each do |row|
      # если исх.строка не пустая
      if row.present?

        # сохраняем текущую строку в хэш
        params = {}
        row.to_h.each do |r|
          params[r[0]] = r[1][r[0]]
        end

        # без названия мероприятия - пропускаем
        if params['subject'] == 0 || params['subject'] == nil || params['subject'] == ''
          break_count += 1
          next
        end
        ...
        is_new_record = false
        # если не существует - создаем новую запись, иначе возвращаем найденную запись
        task = WorkPackage.where(:project_id => @project_for_load.id, :plan_num_pp => params['plan_num_pp'], :subject => params['subject'].to_s[0..250]).first_or_create! do |wp|
          is_new_record = true
          wp.subject = params['subject'].to_s[0..250]
          #wp.plan_num_pp = params['plan_num_pp']
        ...
        # обновление записи с занесением в журнал
        if !is_new_record
          ...
          if params['start_date'].present?
            if params['start_date'] == Date.parse('1899-12-30')
              ...
              # params['start_date'] = Date.parse(params['start_date'].to_s)
              params['start_date'] = Date.strptime(params['start_date'].to_s, "%d.%m.%y")
            end
          else
            params['start_date'] = @project_for_load.created_on
          end
          ...
          params.delete('id')
          params.delete('prev_id')
          params.delete('next_id')
        ...
        rels[params['id']] = Hash['task_id', task.id, 'prev_id', params['prev_id'], 'next_id', params['next_id']]
    ...
    rels.each do |id, rel|
      if rel['prev_id'].present?
        parent_id = rels[parse_prev_id(rel['prev_id'])]['task_id']
      end

      # if rel['next_id'].present?
      #
      # end

      #insert into relations
      if parent_id.present?
        Relation.find_or_create_by(from_id: parent_id, to_id: rel['task_id']) do |r|
          r.from_id = parent_id
          r.to_id = rel['task_id']
          r.hierarchy = 1
        end
  ...
  # добавить юзера в члены проекта
  ...
  def parse_prev_id(prev_id)
    result = ""
    #get only first prev
    if !prev_id.scan(/[;НО]+/).empty?
      result = prev_id.split(/[;НО]+/)[0]
    else
      result = prev_id
    end
    result
