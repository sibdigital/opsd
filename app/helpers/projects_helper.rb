...
module ProjectsHelper
  ...
  # knm +
  # Данный метод действует так же как и метод options_for_select из form_options_helper-a
  # но для данного метода обязательно наличие в наборе данных трех полей
  # первое является тайтлом для выборов
  # последнее является значением выбранного поля
  # и 2 записывается как идентификатор выбора в селекте
  # дальнейшая обработка происходит за счет джаваскрипта
  def children_options_for_select(container, selected = nil)
