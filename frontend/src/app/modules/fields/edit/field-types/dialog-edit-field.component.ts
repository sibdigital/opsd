...
export class DialogEditFieldComponent extends EditFieldComponent implements OnInit {
  @ViewChild('myselect') public myselect: any;
  ...
  private setValues(availableValues: HalResource[]) {
    ...
    this.shownOptions = this.options.slice(1);
  ...
  chooseOrganization() {
    const dialogRef = this.dialog.open(SelectOrganizationDialog, {
      data: {
        items: this.shownOptions,
