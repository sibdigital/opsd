...
export class CustomCellEditorComponent extends DefaultEditor implements OnInit, OnDestroy {
    ...
    protected callOnComponentInit() {
      ...
      this.customComponent.instance.editcomplete.subscribe((event:any) => this.editcomplete.emit(event));
      this.customComponent.instance.canceledit.subscribe((event:any) => this.canceledit.emit(event));
      this.customComponent.instance.cellclick.subscribe((event:any) => this.cellclick.emit(event));
    }
}
