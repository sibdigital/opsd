...
@Component({
  selector: 'wp-document-autocomplete-upgraded',
  templateUrl: './wp-document-autocomplete.upgraded.html'
})
export class WpDocumentAutocompleteComponent {
  ...
  openDialog(): void {
    const modal = this.opModalService.show<WpDocumentConfigurationModalComponent>(WpDocumentConfigurationModalComponent, {projectId: this.projectId});
    modal.closingEvent.subscribe((modal: WpDocumentConfigurationModalComponent) => {
      if (modal.selectedWp) {
