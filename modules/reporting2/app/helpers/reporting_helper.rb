...
module ReportingHelper
  ...
  def show_result(row, unit_id)
    case row.class.engine.to_s
    when "CostQuery" then
    ...
    when "TargetQuery" then
      ...
      #row
      #row.value[0].value[0].value[0].value['plan_value']
      #"  fact: " + row.value[0].value[0].value[0].value['value']
    else
      #row.units #pluralize(row.units, "", "")
      row
    end
  end
