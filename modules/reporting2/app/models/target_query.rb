...
class TargetQuery < Report
  ...
  User.before_destroy do |user|
    ...
    TargetQuery.where(['user_id = ?', user.id]).update_all ['user_id = ?', DeletedUser.first.id]

    max_query_id = 0
    while((current_queries = TargetQuery.limit(1000)
      ...
      current_queries.each do |query|
        ...
        serialized[:filters] = serialized[:filters].map do |name, options|
          options[:values].delete(user.id.to_s) if ["UserId"].include?(name)
        ...
        TargetQuery.where(["id = ?", query.id]).update_all ["serialized = ?", YAML::dump(serialized)]
